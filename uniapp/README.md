...
<template>
	<u-popup :show="show" @close="closeHandler">
		<view class="u-picker">
			...
			<picker-view v-if="innerIndex.length > 0 && innerColumns.length > 0" class="u-picker__view"
				:indicatorStyle="`height: ${$u.addUnit(itemHeight)}`" :value="innerIndex" :immediateChange="immediateChange"
				:style="{
...
export default {
	...
	watch: {
		...
		// 监听默认索引的变化，重新设置对应的值（后定义，后执行，确保在columns之后）
		defaultIndex: {
			immediate: true,
			handler(n) {
	...
	methods: {
		...
		changeHandler(e) {
			...
			// 关键修复：只更新lastIndex用于下次对比，不更新innerIndex
			// 因为picker-view是原生组件，value属性是单向绑定
			// 如果在这里更新innerIndex，会导致picker-view重新渲染，造成滚动异常
			this.setLastIndex(value)
			// 同步更新innerIndex，但避免触发视图重新渲染picker-view
		...
		setColumns(columns) {
			...
			// 修复：确保innerIndex长度与列数一致，处理列数变化的情况
			if (this.innerIndex.length === 0) {
				// 只有在innerIndex为空时才初始化，避免覆盖defaultIndex的设置
				this.innerIndex = new Array(columns.length).fill(0)
				this.lastIndex = new Array(columns.length).fill(0)
			} else if (this.innerIndex.length !== columns.length) {
				// 长度不匹配时，保留已有的值，不足部分补0
				const oldIndex = this.innerIndex
				this.innerIndex = new Array(columns.length).fill(0).map((_, i) => oldIndex[i] || 0)
				this.lastIndex = uni.$u.deepClone(this.innerIndex)
			}
		},
		// 获取各列选中值对应的索引
		getIndexs() {
