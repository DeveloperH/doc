...
官网：https://github.com/debauchee/barrier



在 Barrier 的「屏幕设置」中，针对 Mac 客户端，建议按如下方式修改右侧的 “修改按键”：

| Barrier 选项 | 推荐映射值 | 说明                                                         |
| ------------ | ---------- | ------------------------------------------------------------ |
| **Shift**    | `Shift`    | 保持不变，用于输入上档字符（如 `Shift+,` 输入 `<`）          |
| **Ctrl**     | `Meta`     | 将 Windows 的 Ctrl 键映射为 Mac 的 Command 键，用于 Cmd+C/V/X 等复制粘贴操作 |
| **Alt**      | `Alt`      | 将 Windows 的 Alt 键映射为 Mac 的 Option 键                  |
| **Meta**     | `Control`  | 将 Windows 的 Win 键映射为 Mac 的 Control 键，用于 Control + 空格切换输入法等 |
| **Super**    | `Super`    | 保持默认即可，一般无需修改                                   |

选择 Mac 的那个屏幕图标，修改保存后重启服务端。



1. **复制粘贴**
   - Windows 端：`Ctrl + C` / `Ctrl + V` / `Ctrl + X`
   - 映射到 Mac 端：`Command + C` / `Command + V` / `Command + X`，完全符合 Mac 的使用习惯。
2. **Shift 输入小于号 `<`**
   - Windows 端：`Shift + ,`（逗号键）
   - 映射到 Mac 端：`Shift + ,`，直接输出 `<`，无需额外配置。
3. **切换输入法**
   - Windows 端：`Win + 空格`
   - 映射到 Mac 端：`Control + 空格`，这是 Mac 系统默认的输入法切换快捷键。
