...
* `/usr` : 存放着一些用户自己安装的软件，类似于windows下的Program Files目录。
* `/var` : 存放着Linux下的一些日志文件，在实际开发的时候有一些公司也习惯把Apache或者Nginx的站点目录也放到这个目录。`/var/www`



目前常用的Linux远程登录工具有：Putty、Xshell、SecureCRT等等。
...
@符号：表示"在"
localhost：表示当前的主机名字
波浪号~：表示当前的工作路径，~在Linux中表示用户的家目录。
#符号：表示判断当前用户的身份，即用户身份的标记。#表示超级管理员用户，$表示普通用户。使用su admin命令来切换用户。
```
...
| 目录       | 核心用途                     | 生活类比         | 常见内容                                     |
| :--------- | :--------------------------- | :--------------- | :------------------------------------------- |
| **/bin**   | 存放系统启动所需的基本命令   | **工具箱**       | ls, cp, cat, sh                              |
| **/boot**  | 存放启动引导程序和内核       | **引擎启动器**   | vmlinuz, grub                                |
| **/dev**   | 存放硬件设备文件             | **设备插槽**     | /dev/sda, /dev/tty                           |
| **/etc**   | 存放系统及其服务的配置文件   | **设置说明书**   | passwd, fstab, hosts                         |
| **/home**  | 存放普通用户的主目录         | **员工房间**     | /home/user1, /home/user2                     |
| **/lib**   | 存放/bin和/sbin所需的库文件  | **公共图书馆**   | [ld-linux.so](https://ld-linux.so/), .so文件 |
| **/media** | 自动挂载可移动设备的挂载点   | **自动泊车区**   | U盘、光盘                                    |
| **/mnt**   | 临时手动挂载文件系统的挂载点 | **临时停车位**   | 外部硬盘、ISO镜像                            |
| **/opt**   | 存放第三方或可选的应用软件   | **扩充应用区**   | oracle, google-chrome                        |
| **/proc**  | 虚拟目录，存放内核与进程信息 | **监控仪表盘**   | cpuinfo, meminfo                             |
| **/root**  | 超级管理员root用户的家目录   | **经理办公室**   | root的配置文件                               |
| **/run**   | 存放系统运行时产生的临时数据 | **运行工作台**   | PID文件、Unix套接字                          |
| **/sbin**  | 存放系统管理命令             | **维修工具间**   | shutdown, fdisk, reboot                      |
| **/srv**   | 存放由本系统提供的服务数据   | **服务数据室**   | Web、FTP服务器数据                           |
| **/sys**   | 虚拟目录，存放内核设备信息   | **系统状态面板** | 硬件设备、驱动程序信息                       |
| **/tmp**   | 存放临时文件，重启后会被清除 | **公共白板**     | 缓冲文件、临时下载                           |
| **/usr**   | 存放用户级应用程序和数据     | **应用商店**     | /usr/bin, /usr/share                         |
| **/var**   | 存放系统运行中频繁变化的数据 | **日志储藏室**   | 日志、邮件、缓存                             |



 **几个需要区分的重点：**

1. **/bin 和 /sbin**：**/bin** 里的命令是面向所有用户的，即使系统进入故障恢复模式也必不可少。而 **/sbin** 里的命令通常只有系统管理员（root用户）才能执行，主要用于管理硬件和系统状态。
2. **/usr 和 /home**：**/usr** 是系统安装的软件和资源的存放地，而 **/home** 是用户私人数据的存放地。在现代的某些Linux发行版中，/bin、/sbin 和 /lib 实际上是指向 /usr 目录下对应位置的快捷方式，这种设计让系统文件更加统一。
3. **/dev 与 /proc、/sys**：这几个目录里的东西并不真正存在于硬盘上。**/dev** 是硬件设备的“接口”，**/proc** 和 **/sys** 则记录了系统运行时的内存和内核状态。
4. **/mnt 和 /media**：两者都是用来挂载外部设备的，**/media** 通常用于系统自动识别的U盘等，**/mnt** 则更倾向于让管理员手动挂载临时文件系统。
...
`passwd [需要修改的用户名]` ：用来更改使用者的密码，需要根据提示输入一次旧密码和两次新密码。
...
用户组添加完成之后也不会有任何提示，此时我们可以查看用户组的管理文件，文件名叫做 group，位置位于 `/etc/group`
...
如果这个用户组中存在用户的话，则该用户组是无法直接删除的。如果需要删除的话，则可以先将组内的成员删除掉，或者移动到别的组，然后才可以删除这个用户组。
...
* 所有的用户添加之后都会存储在 passwd 文件中，位置位于 `/etc/passwd`。
...
* `rpm -qa[|grep] 需要查询的关键词` : 查询安装
  * `-q` : 表示query，查询
  * `-q` : 表示全部，all
  * `-q` : 管道
  * `grep` ：表示从结果中进行过滤
  * 例如：可以使用 `rpm -qa firefox` 查询火狐的安装情况。
* `rpm -e 需要卸载的软件名称(软件完整名称，即通过查询得到的名称) [--nodeps]` : 软件的卸载
  * `-e` : 表示卸载
  * `--nodeps` : 表示忽略依赖关系
  * 例如：卸载火狐 `rpm -e 查询到的软件全名`
* `rpm -ivh 需要安装的软件完整名称` : 安装软件
  * `-i` : 表示安装，install
  * `-v` : 表示显示进度条
  * `-h` : 表示进度条以 `#` 号显示
...
1表示单用户模式；
2表示多用户模式；
3表示将Linux系统从桌面模式切换到命令行模式；
4表示未被使用的模式(暂时没用)；
5表示将Linux系统从命令行模式切换到桌面模式；
6表示重启。
```
...
命令：`mount [参数] 设备的名称 挂载点/挂载位置`

例如：`mount /dev/sr0 /mnt/dvd`

设备名称可以通过命令 `#lsblk`（list block devices）来获取。使用该命令前，先确保 iso 文件已经放入光驱中。在列出的文件数中找到最接近光驱大小的设备名：sr0，在lsblk列出的块状设备都位于 `/dev` 目录中，所以 sr0的完整路径应该是 `/dev/sr0`



所谓光盘挂载，可以理解成将光盘创建一个快捷方式。在windows下也有类似这样的过程（盘符的分配），但Linux下必须手动的进行光盘/USB外接设备的挂载，只有挂载之后才能使用该外接设备中的文件/文件夹。

挂载点就是指将这个设备放到什么地方去，挂载点就是一个目录。一般情况下有个习惯，在实际使用的时候习惯将外接设备挂载到 `/mnt` 目录下，比如说可以在 mnt 目录下创建一个 dvd 目录，然后将光盘挂载到 `/mnt/dvd` 上。创建好目录之后再去使用命令进行挂载。

执行命令后，显示提示(该块有写保护，只读)，表示成功挂载，可以在 `/mnt/dvd` 目录中访问了。
